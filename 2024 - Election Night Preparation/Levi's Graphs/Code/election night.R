...
mapANDresults_2023 %>%
  ...
  labs(title = "2023 Early In-Person Voting by Precinct", 
       ...
       fill = "Early Voting (%)") +
  scale_fill_gradientn(colours = brewer.pal(n = 6, name = "Greens"), na.value = "transparent",
                       breaks = c(0, 5, 10, 15, 20, 25, 30), labels = c("0%", "5%", "10%", "15%", "20%", "25%", "30%"),
                       limits = c(0, 30)) +
  geom_sf(data = board_of_elections, color = 'pink', size = 6, shape = 4, inherit.aes = FALSE) + # Adding the Board of Elections point
...
mapANDresults_2020 %>%
  ...
  scale_fill_gradientn(colours = brewer.pal(n = 6, name = "Greens"), na.value = "transparent",
                       breaks = c(0, 5, 10, 15, 20, 25, 30), labels = c("0%", "5%", "10%", "15%", "20%", "25%", "30%"),
                       limits = c(0, 30)) +
  geom_sf(data = board_of_elections, color = 'pink', size = 6, shape = 1, lwd = .1, inherit.aes = FALSE) + # Make Board of Elections marker thicker by adjusting the line width

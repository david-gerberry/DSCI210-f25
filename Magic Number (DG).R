...
total_voters <- 100   #Use smaller number so simulation actually finishes
percentages <- c(0.313, 0.044, 0.6426)  # Votes distribution

# Calculate the number of votes
no_votes <- round(total_voters * percentages[1])  # No votes
one_vote <- round(total_voters * percentages[2])  # Votes for one candidate
two_votes <- round(total_voters * percentages[3])  # Votes for two candidates

# Define candidates
candidates <- c("A", "B", "C", "D", "E")

# Function to simulate the election
simulate_election <- function(candidates, one_vote, two_voters) {
...
total_voters <- 100
C = 5    ## number of candidates
winners=2 
j = 1

simulate_election_DG <- function(candidates =  LETTERS[1:C], winners=2, j=1,total_voters =100) {
  # Create a ballot list
  ...
  for (i in 1:total_voters){
    ...
    ballots[[i]] <- sample(x=candidates,size=winners,prob = (p+j)/sum(p+j))
  }
...
# Store results
results <- replicate(79, simulate_election_DG())
...
results_df$MagicNumber <- apply(results_df[, 1:5], 1, function(x) (sort(x, decreasing = TRUE)[winners]+sort(x, decreasing = TRUE)[winners+1])/2)

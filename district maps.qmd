...
x = mapANDresults2019
	...
	ind = which((x$`Josh Berkowitz`+x$`John Kennedy`)>0)		
		y = x[ind,] %>% select(PRECINCT,`PRC #`,geometry)

		judicial_boundary <-  st_union(y)
		judicial_precincts <- st_intersection(st_transform(map2025, st_crs(y)),judicial_boundary)
...
x <- mapANDresults2021
    ...
    ind = which((x$`David              Mann`+x$`Aftab        Pureval`)>0)		
		...
		cincy_precincts <- st_intersection(st_transform(map2025, st_crs(y)),cincy_boundary)
...
cps_shape <- st_zm(st_read("data/maps/tl_2021_39_unsd.shp")) %>% 
...
cps_boundary <-  st_transform(cps_shape, st_crs(map2021))
cps_precincts <- st_intersection(st_transform(map2025, st_crs(map2021)),cps_boundary)

    #   cps_precincts <- st_intersection(y,cps_boundary) %>% 
    #     rename(`PRC #` = PRC..) %>%  
    # 		  select(PRECINCT,`PRC #`,geometry)
    # 		
